...
export function NavbarMobile({ resumeData }: NavbarMobileProps) {
  ...
  return (
    <div className="md:hidden flex items-center">
      ...
      {/* Fullscreen Overlay */}
      <div 
        className={`fixed inset-0 top-[64px] z-40 bg-bg-page/95 backdrop-blur-xl transition-all duration-300 ease-in-out ${
        ...
        <div className="flex flex-col h-full p-6 pb-24 overflow-y-auto">
          <nav className="flex flex-col gap-6 mt-8">
            ...
            {resumeData?.resumeFileUrl && (
              <a href={resumeData.resumeFileUrl} download onClick={closeMenu} className="flex justify-center items-center gap-2 py-4 rounded-full border border-default bg-bg-surface1 font-medium text-text-primary hover:text-accent-indigo transition-all">
