  ...
  ({ className = '', variant = 'primary', ...props }, ref) => {
    ...
    switch (variant) {
      ...
      case 'ghost':
        ...
        break;
      case 'icon':
        variantStyles = 'bg-transparent border border-border-default hover:border-border-hover text-text-primary w-10 h-10 flex items-center justify-center rounded-full p-0';
        break;
    }
    ...
    return (
      <button
        ref={ref}
        className={`px-4 py-2 font-sans transition-all duration-300 ${variantStyles} ${className}`}
        {...props}
      />
    );
  }
